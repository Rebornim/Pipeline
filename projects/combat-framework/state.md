...
## Current Stage: Pass 10 Exit Fix v3 — Ready to Build
## Status: v2 fixes applied and confirmed working (Jump block, sync verified). New bug: auto-land descends to ground but settled check never fires — player stuck in ship. Root cause: settled check uses `abs(altError) < 0.8` but PrimaryPart center can't reach within 0.8 studs of target because collision geometry rests on ground with center higher up. Virtual vertical speed also non-zero (no physics feedback). Fix: replace altError+virtualSpeed check with `altError > -8` + `horizontalVelocity < 1` + `AssemblyLinearVelocity.Magnitude < 2` (actual physics velocity). Design in `pass-10-exit-fix-v3-design.md`.
...
- **Pass 10 Fix Design (replication):** Complete 2026-02-25. Mid-pass live-server audit via critic agent found 8 root causes (5 blocking) for Studio-vs-live divergence. Fix design in `pass-10-fix-design.md` — 6 build steps targeting replication layer. **Applied but had ZERO effect — wrong layer. Problem is local pilot physics, not replication.**
- **Pass 10 Physics Fix Design:** Complete 2026-02-25. User reported exact symptoms: nose-down death spiral on takeoff, unresponsive pitch, slow accel, camera drift. Diagnosed to network ownership race (client drives BodyMovers before SetNetworkOwner arrives — invisible in Studio where client=server) compounded by inverted ground collision pitch (`math.rad(-5)` = nose DOWN instead of UP) and insufficient lift at minSpeed. Fix design in `pass-10-physics-fix-design.md` — 5 steps: ownership probe gate, collision pitch sign fix, lift rebalance (0.22→0.32), physics feedback sync, camera speed attr restore.
- **Pass 10 Physics Fix Build:** Applied 2026-02-26. Fixed landing→flight death spiral. Remaining live-server symptoms: slow physics (dt clamp), stuck vertical mouse, diagonal velocity on turns (virtual orientation divergence), camera drift. Critic audit confirmed 3 blocking issues: dt clamp time loss, missing orientation feedback, SetNetworkOwner re-assertion disruption.
- **Pass 10 Flight Fix Design:** Complete 2026-02-26. Critic-reviewed fix for remaining live-server flight issues. 4 steps: (1) remove client VehicleSpeed double-write + camera reads AssemblyLinearVelocity directly, (2) orientation feedback blending physicsOrientation toward base.CFrame.Rotation, (3) fixed-step physics accumulator (1/60s substeps, max 4) replacing dt clamp, (4) downgrade ensurePilotOwnership to log-only with 2s interval. Design in `pass-10-flight-fix-design.md`.
- **Pass 10 Flight Fix Build:** Applied 2026-02-26. Marginal improvement but core issues persist. Flight still sluggish, diagonal velocity on turns, camera drift, mouse stuck on vertical.
- **Pass 10 Force-Fix Design:** Complete 2026-02-26. Deep critic audit found the smoking gun: `fighterUseForceFlight = true` activates VectorForce code path where FighterClient line 1542 (`physicalVelocity = base.AssemblyLinearVelocity`) discards the entire virtual flight simulation's output every frame. In Studio, AssemblyLinearVelocity reflects force writes instantly (same process) — snap reads correct value. In live, it lags 1-3 physics frames — snap reads stale value, virtual sim always chases reality from behind. Explains all remaining symptoms. Fix: `fighterUseForceFlight = false` (one config line) switches to BodyVelocity path where virtual sim leads and BodyVelocity follows (no snap). Design in `pass-10-force-fix-design.md`.
- **Pass 10 Force-Fix Build:** Applied 2026-02-26. Marginal improvement. Flight still 5x slower in live vs Studio. Mouse still stuck at times. Turns still sluggish. Config change was correct but not sufficient — the real problem is elsewhere.
- **Pass 10 Accumulator Fix Design:** Complete 2026-02-26. Fifth deep critic audit found the DEFINITIVE root cause: the fixed-step accumulator added in the flight-fix round uses `math.max(1, substepCount)` which forces one physics step per frame at `stepDt = PHYSICS_FIXED_STEP = 1/60` regardless of real frame time. At FPS > 60, physics runs at FPS/60 speed. Studio at ~300fps = 5x real-time. Live at ~60fps = 1x real-time. Exact 5:1 ratio matches user's "5x slower" report. Secondary: velocity feedback (lines 1554-1567) fights turns in live (divergence > 2 triggers), never fires in Studio (BV tracks perfectly at high FPS). Orientation feedback rate 3.0 too aggressive for live. Fix: 4 steps — (1) remove accumulator, use `stepDt = math.clamp(dt, 1/240, 1/20)`, (2) delete velocity feedback entirely, (3) reduce orientation feedback rate 3.0→1.5, (4) clean stale prints. Design in `pass-10-accumulator-fix-design.md`.
- **Pass 10 Accumulator Fix Build:** Applied 2026-02-26. Movement confirmed working — Studio and live now match. User reports "movement works as normal."
- **Pass 10 Landing Fix Design:** Complete 2026-02-26. Two confirmed bugs: (1) ProximityPrompt vanishes after exit — `releaseFighterMomentum()` unanchors ship, it falls away. (2) Mouse locked on pitch after landing→takeoff — during 0.9s takeoff phase, cursor processing still runs, mouse deltas accumulate `virtualCursorY` to circle boundary edge. Player must move opposite direction first. Fix: 4 steps — (1) cursor reset on takeoff→flight completion, (2) block F exit during flight mode (only during landing), (3) new auto-land exit system: F during landing triggers smooth horizontal deceleration + raycast ground descent, fires exit when grounded+stopped, (4) server always calls `parkFighter()`. Design in `pass-10-landing-fix-design.md`.
